...
class kodi(object):
    ...
    def call_api(self, method, params=None):
        ...
        if params:
            data['params'] = params
        url = "http://%s:%d/jsonrpc?request=" % (
            self.config["host"].lower(), self.config["port"]) + json.dumps(data)
        ...
        if len(active):
            for player in active:
                ...
                if ((not restart) and (speed > 0)) or ((restart) and (speed == 0)):
                    params = {"playerid": player['playerid']}
                    if self.call_api("Player.PlayPause", params).get('result', None) is not None:
                        if restart:
                            self.unmute()
                        self.response_message = "OK %s" % (name)
                    else:
    ...
    def give_help(self):
        self.response_message = \
            "To play music say: %s play, followed by the album or song title. " % \
            (self.keyword) + \
            "To control playback use %s stop, %s pause or %s restart. " % \
            (self.keyword, self.keyword, self.keyword) + \
            "You can also use %s next or %s previous to change track when playing an album." % \
...
def main(config_file, keyword, params):
    ...
    action = params[0].lower()
    {
        ...
        'mute': k.mute, 'unmute': k.unmute, 'previous': k.play_previous, 'next': k.play_next
    }.get(action, k.fallback)()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        config_file = os.path.join(os.path.dirname(sys.argv[0]), "kodi_config.json")
        print("%s" % (main(config_file, sys.argv[1], sys.argv[2:])))
